...
//      gui->autosave = m_config.opt_string("autosave");
        ...
        wxMessageBox(boost::nowide::widen(ex.what()), _L("OpenStudio GUI initialization failed"), wxICON_STOP);

    } catch (const std::exception &ex) {
        if (is_log_trivival_valid()) {
            BOOST_LOG_TRIVIAL(error) << ex.what() << std::endl; // boost log not initialized yet 
        }
        wxMessageBox(format_wxstr(_L("Fatal error, exception caught: %1%"), ex.what()), _L("OpenStudio GUI initialization failed"), wxICON_STOP);
    }
    // error
    return 1;
}
